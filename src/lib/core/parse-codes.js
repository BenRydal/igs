export class ParseCodes {
	...
	updateParsedFileArrayForMultiCodes(results) {
		for (const row of results.data) {
			if (this.coreUtils.codeRowForType(row)) {
				// IMPORTANT: in case there is partial missing data etc.
				...
				for (const codeTable of this.parsedFileArray) {
					// test for name match to updated existing codeTable
	...
	getCodeData(curTime) {
		...
		for (let i = 0; i < this.parsedFileArray.length; i++) {
			const curCodeTableRow =
				this.parsedFileArray[i].parsedCodeArray[this.parsedFileArray[i].counter];
			if (this.coreUtils.codeRowForType(curCodeTableRow)) {
				// IMPORTANT: in case there is partial missing data etc.
				if (this.timeIsBetweenCurRow(curTime, this.parsedFileArray[i])) {
					...
					if (
						this.parsedFileArray[i].counter < this.parsedFileArray[i].parsedCodeArray.length - 1 &&
						this.timeIsBetweenNextRow(curTime, this.parsedFileArray[i])
					) {
						hasCodeArray.push(true);
						color = this.getCodeColor(color, i);
	...
	timeIsBetweenCurRow(curTime, codeTable) {
		return this.between(
			curTime,
			this.getStartTime(codeTable.parsedCodeArray, codeTable.counter),
			this.getEndTime(codeTable.parsedCodeArray, codeTable.counter)
		);
	}
	...
	timeIsBetweenNextRow(curTime, codeTable) {
		return this.between(
			curTime,
			this.getStartTime(codeTable.parsedCodeArray, codeTable.counter + 1),
			this.getEndTime(codeTable.parsedCodeArray, codeTable.counter + 1)
		);
	}
