...
export class SetPathData {
    ...
    constructor(sketch: any, codeList: string[]) {
        this.sk = sketch;
    ...
    setMovement(users: Map<number, string>) {
        const drawMovement = new DrawMovement(this.sk, this.drawUtils);

        // users.forEach((values: , keys: number) => {
    ...
    // TODO:
    // Double check use case/why these are 2 separate pieces & why this
    // must be different to just setMovement
