...
let myp5 = new p5((igs) => {
     ...
     */
    igs.setup = () => {
        igs.canvas = createCanvas(window.innerWidth, window.innerHeight, P2D);
        igs.frameRate(30);
        core = new Core();
        setData = new SetData();
    ...
    igs.draw = () => {
        igs.background(255);
        if (testData.dataIsLoaded(core.floorPlan)) igs.image(core.floorPlan, 0, 0, keys.displayFloorPlanWidth, keys.displayFloorPlanHeight);
        ...
        if (testData.dataIsLoaded(videoPlayer) && core.isModeVideoShowing && (igs.mouseX !== igs.pmouseX || igs.mouseY !== igs.pmouseY)) {
            if (!core.isModeVideoPlaying) setData.setVideoScrubbing();
            select('#moviePlayer').position(igs.mouseX - videoPlayer.videoWidth, igs.mouseY - videoPlayer.videoHeight);
        }
